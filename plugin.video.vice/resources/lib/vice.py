...
from tulip import bookmarks, cache, client, control, directory
...
class indexer:
    ...
    def __init__(self):
        ...
        self.shows_link = '/{0}/shows/'
        self.episode_link = '/{0}/playlists/{1}/tab1?page=1'
        self.ooyala_link = 'http://player.ooyala.com/player/ipad/{0}.m3u8'
        self.ooyala_subtitle_link = 'https://player.ooyala.com/nuplayer?embedCode={0}'
        self.uplynk_link = 'https://video.vice.com/en_us/preplay/{0}?tvetoken=&rn=&exp={1}&sign={2}'
        ...
        self.list = [
            {
                'title': 32001,
                'action': 'videos',
                'url': self.shows_link.format(self.country),
                'icon': 'topvideos.png'
            }
            ,
            {
    ...
    def countries(self):
        ...
        for i in self.countryDict:
            ...
            self.list.append({'title': i['title'], 'url': self.shows_link.format(i['code'])})
    ...
    def item_list_5(self, url, country):
        ...
        try:
            ...
            if '/shows/' in url:
                ...
                u = [i for i in urlparse.urlparse(url).path.strip('/').split('/')]
                url = self.episode_link.format((u[0], u[-1]))
        ...
        for item in items:
            ...
            try:
                ...
                try:
                    tvshowtitle = item['show']['base']['display_title']
                except:
                    tvshowtitle = '0'
                ...
                try:
                    image = item['lede_image']['image_url']
                except:
                    image = '0'
                ...
                try:
                    duration = (60 * int(d[0])) + int(d[1])
                except:
                    pass
                try:
                    duration = (60 * 60 * int(d[0])) + (60 * int(d[1])) + int(d[2])
                except:
                    pass
                ...
                data = {'title': title, 'tvshowtitle': tvshowtitle, 'url': url, 'image': image, 'fanart': image, 'duration': duration, 'plot': plot, 'next': next}
    ...
    def item_list_6(self, country):
        ...
        for item in items:
            try:
                ...
                url = self.episode_link.format(country, url)
                url = url.encode('utf-8')
    ...
    def resolve(self, url):
        ...
        try:
            ...
            if yid:
                return 'plugin://plugin.video.youtube/play/?video_id={0}'.format(yid[0]), None
            ...
            if upid:
                ...
                sig = '{0}:GET:{1}'.format(upid, exp)
                sig = hashlib.sha512(sig.encode()).hexdigest()
                url = self.uplynk_link.format(upid, exp, sig)
    ...
    def settings(self):
        ...
        try:
            self.country = setting[0]['code']
        except:
            self.country = 'en_us'

        try:
            self.lang = setting[0]['iso']
        except:
            self.lang = ''
